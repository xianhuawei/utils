...
def get_user_server_list():
    ...
    try:
        result={}
        for item in res:
            key = item['datacenter']+'->'+item['business']+'->'+item['container']+'->'+item['module']
    ...
    def save(self):
        if is_log2db:
            ...
            db.update(
                ...
                % (self.is_finished, self.end_time, self.filename, self.insert_id))
...
class ExecLog():
    def __init__(self, host, cmd, remote_ip, result):
        ...
        if is_log2db:
            try:
                db._ensure_connected()
                db.insert(sql, user, host, MySQLdb.escape_string(cmd), remote_ip,
