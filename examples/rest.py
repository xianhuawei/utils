...
class APIKeyAuthentication(Authentication):
    ...
    def get_key_secret(self):
        for search in [Request._params, Request.headers]:
    ...
    def authorize(self):
        ...
        if key or secret:
            self.api_key = self.get_key(key, secret)
...
class UserAuthentication(Authentication):
    ...
    def authorize(self):
        ...
        self.user = self.auth.authenticate(basic_auth.username, basic_auth.password)
        return self.user
...
class AdminAuthentication(UserAuthentication):
    ...
    def authorize(self):
        ...
        if res and self.user:
            return self.verify_user(self.user)
        return res
...
class RestResource(object):
    ...
    def process_query(self, query):
        ...
        for key in Request._params:
            ...
            raw_filters[expr].append((op, Request._params.getlist(orig_key), negated))
    ...
    def response_forbidden(self):
        return falcon.HTTPForbidden('Forbidden')

    def response_bad_method(self):
        return falcon.HTTPMethodNotAllowed(Request.method)

    def response_bad_request(self):
        return falcon.HTTPBadRequest('Bad request')

    def response(self, data):
        kwargs = {} if Request.get_header('X-Requested-With') == 'XMLHttpRequest' else {'indent': 2}
        Response.body = json.dumps(data, **kwargs)
    ...
    def get_request_metadata(self, paginated_query):
        ...
        request_arguments = Request._params.copy()
    ...
    def get_paginate_by(self):
        try:
            paginate_by = int(Request._params.get('limit', self.paginate_by))
        except ValueError:
    ...
    def object_list(self):
        ...
        if self.paginate_by or 'limit' in Request._params:
...
class RestrictOwnerResource(RestResource):
    ...
    def check_put(self, obj):
        return self.validate_owner(self.user, obj)

    def check_delete(self, obj):
        return self.validate_owner(self.user, obj)

    def save_object(self, instance, raw_data):
        self.set_owner(instance, self.user)
