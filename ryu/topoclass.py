...
class Link(object):
    ...
    def to_dict(self):
        d = {'src': self.src.to_dict(), 'dst': self.dst.to_dict()}
        return d
...
#    idle_timeout=0, hard_timeout=0, actions=actions)
    ...
    def install_flow_dip(self, dp, sport, dport, dip, actions=None):
        ...
        actions = [dp.ofproto_parser.OFPActionOutput(dport)]
        ofproto = dp.ofproto
        match = dp.ofproto_parser.OFPMatch(in_port=sport,
                                           dl_type=0x0800,
                                           nw_dst=dip)
        mod = dp.ofproto_parser.OFPFlowMod(
            datapath=dp,
    ...
    def create_arp_response(self, srcip, srcmac, dstip):
        ...
        arp_packet = arp.arp(src_mac=src,
                             dst_mac=dst,
                             dst_ip=srcip,
                             src_ip=dstip,
    ...
    def create_graph_map(self):
        for link in self.links._map:
            ...
            self.graph_ports[link.dpid][self.links._map[
                link].dpid] = link.port_no
        ...
        for p in pkt:
            ...
            if p.protocol_name == 'lldp':
                ...
                if old_peer and old_peer != dst:
                    ...
                    self.send_event_to_observers(event.EventLinkDelete(
                        old_link))
            ...
            if p.protocol_name == 'arp':
                ...
                if (dst_ip_addr in self.arp_table):
                    print('response')
                    arp_pkt = self.create_arp_response(
                        src_ip_addr, src_mac_addr, dst_ip_addr)
            ...
            if p.protocol_name == 'ipv4':
                ...
                self._drop_packet(msg)
                return
            if (p.protocol_name != 'arp' and p.protocol_name != 'lldp' and
                    p.protocol_name != 'ethernet'):
                print(p)
